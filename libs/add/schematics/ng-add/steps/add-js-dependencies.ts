...
export function addJsDependencies(options: Schema): Rule {
  return async (tree: Tree, context: SchematicContext) => {
    return updateWorkspace((workspace: WorkspaceDefinition) => {
      ...
      logger.info(`🔄 Adding "@sijil/core" library to "allowedCommonJsDependencies"...`);
      ...
      try {
        ...
        if (!project) {
          logger.error(
            `❌ Target project not found. Please add "@sijil/core" library to "allowedCommonJsDependencies" manually.`,
          );
        ...
        targetOptions['allowedCommonJsDependencies'] = Array.from(
          new Set([...(jsDependencies ?? []), '@sijil/core']),
        );
